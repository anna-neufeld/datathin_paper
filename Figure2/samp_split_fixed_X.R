...
full.results.ss <- array(NA, dim=c(nTrials*2, p, 2)) 
full.results.datathin <-  array(NA, dim=c(nTrials*2, p, 2)) 

### A few outliers have a lot of power. 
X <- matrix(rnorm(n*p), nrow=n, ncol=p)
X[16,] <- rnorm(p, mean=5)

betas <- rep(rep(seq(0.01, 1, length.out=20), nTrials/20), each=2)

epsilons <- rep(c(0.5, 0.8), nTrials)
sigma.sq <- 1
...
res.ss <- data.frame(t(sapply(1:(nTrials*2), function(u) power.check(full.results.ss[u,,], c(rep(betas[u], p.imp), rep(0, p-p.imp))))))
colnames(res.ss) <- paste0("X", 1:p, sep="") 
res.ss$beta <- betas
res.ss$epsilon <- epsilons
res.ss$method <- "Sample splitting"

res.dt <- data.frame(t(sapply(1:(nTrials*2), function(u) 
  power.check(full.results.datathin[u,,], c(rep(betas[u], p.imp), rep(0, p-p.imp))))))
colnames(res.dt) <- paste0("X", 1:p, sep="") 
res.dt$beta <- betas
res.dt$epsilon <- epsilons
res.dt$method <- "Data thinning"

res.FIXED <- rbind(res.ss, res.dt)

save(res.FIXED, file="fixedX_res-nov6-1out.RData")
